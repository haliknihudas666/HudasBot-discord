...
exports.run = async (client, message, [args, time], level) => { // eslint-disable-line no-unused-vars
    ...
    if (!message.channel.nsfw) {
        ...
        if (args === "timed" && message.guild.ownerID === message.author.id) {
            warningEmbed.setDescription(`**Timed post has been activated. New random post from /r/godpussy every ${time} minute(s).**`);
            warningEmbed.setColor('#5cb85c');
            message.channel.send(warningEmbed);

            timed = setInterval(() => {
                client.logger.log("[GODPUSSY] Sent a timed message");
                RedditSimple.RandomPost("godpussy").then(async res => {
                    ...
                    var nsfwEmbed = {
                        ...
                        author: {
                            name: `r/${subreddit}`,
                            icon_url: 'https://b.thumbs.redditmedia.com/AYaEySBcq6IlFGaEMT4ePh1O9RaBN8gtGSrR3blk6xY.png',
